...
const ACTIVITIES = {
  badYogi:   { name: "Bad Yogi",      emoji: "🧘", xp: 50, color: "#C8A882" },
  growWithJo:{ name: "Grow with Jo",  emoji: "🏃", xp: 40, color: "#7BAE7F" },
...
const REWARDS = [
  ...
  { xp: 300,  name: "Fancy Coffee Date (solo!)",        emoji: "☕" },
...
export default function FitnessQuest() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0e17", fontFamily: "'Georgia', serif", color: "#f5f0e8", paddingBottom: 60 }}>
      ...
      {toast && (
        <div style={{ position: "fixed", top: 24, left: "50%", transform: "translateX(-50%)", background: "#C8A882", color: "#0f0e17", padding: "12px 28px", borderRadius: 40, fontWeight: "bold", fontSize: 18, zIndex: 100, boxShadow: "0 4px 30px rgba(200,168,130,0.4)", letterSpacing: 1, whiteSpace: "nowrap" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 20px", position: "relative" }}>
        ...
        <div style={{ textAlign: "center", paddingTop: 48, paddingBottom: 32 }}>
          <div style={{ fontSize: 13, letterSpacing: 4, color: "#C8A882", marginBottom: 8, textTransform: "uppercase" }}>Rae's</div>
          <h1 style={{ fontSize: 42, margin: 0, fontWeight: "normal", letterSpacing: 2, lineHeight: 1.1 }}>Fitness Quest</h1>
          <div style={{ fontSize: 13, color: "#888", marginTop: 8, letterSpacing: 2 }}>10 WEEKS · POSTPARTUM COMEBACK</div>
        </div>

        {/* XP Card */}
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(200,168,130,0.2)", borderRadius: 20, padding: "24px 28px", marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 12, letterSpacing: 3, color: "#888", textTransform: "uppercase", marginBottom: 4 }}>Level {currentLevel.level}</div>
              <div style={{ fontSize: 22, color: "#C8A882" }}>{currentLevel.name}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 36, fontWeight: "bold", color: "#f5f0e8", lineHeight: 1 }}>{totalXP}</div>
              <div style={{ fontSize: 12, color: "#888", letterSpacing: 2 }}>TOTAL XP</div>
            </div>
          </div>
          {nextLevel && (
            <>
        ...
        {/* Goal Banner */}
        <div style={{ background: "linear-gradient(135deg, rgba(123,174,127,0.15), rgba(123,174,127,0.05))", border: "1px solid rgba(123,174,127,0.3)", borderRadius: 16, padding: "16px 24px", marginBottom: 24, display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 32 }}>🏅</div>
          <div>
            <div style={{ fontSize: 11, letterSpacing: 3, color: "#7BAE7F", textTransform: "uppercase" }}>Final Goal</div>
            <div style={{ fontSize: 18 }}>Run a 5K before returning to work</div>
          </div>
          <div style={{ marginLeft: "auto", fontSize: 12, color: "#666" }}>Week 10 🎯</div>
        </div>
          ...
          {weekPlan.map((w, i) => {
            ...
            return (
              <button key={i} onClick={() => setCurrentWeek(i)} style={{
                ...
                cursor: "pointer", fontSize: 13, letterSpacing: 1, transition: "all 0.2s",
              }}>
                <div>Wk {i + 1}</div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  {!d.completed && (
                    <button onClick={() => setLogging(isLogging ? null : { week: currentWeek, day: di })} style={{
                      padding: "5px 14px", borderRadius: 20, border: "1px solid rgba(200,168,130,0.3)",
                      background: isLogging ? "rgba(200,168,130,0.2)" : "transparent",
                      color: "#C8A882", cursor: "pointer", fontSize: 12, letterSpacing: 1,
                    }}>
                  ...
                  <div style={{ marginTop: 14, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
                      ...
                      {Object.entries(ACTIVITIES).map(([k, act]) => {
                        ...
                        return (
                          <button key={k} onClick={() => toggleActivity(currentWeek, di, k)} style={{
                            ...
                            cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s",
                          }}>
                    ...
                    {d.activities.length > 0 && (
                      <button onClick={() => completeDay(currentWeek, di)} style={{
                        ...
                        color: "#0f0e17", cursor: "pointer", fontSize: 14, fontWeight: "bold", letterSpacing: 1,
                      }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontSize: 11, letterSpacing: 4, color: "#888", textTransform: "uppercase", marginBottom: 14 }}>Rewards</div>
            ...
            {REWARDS.map((r) => {
              ...
              return (
                <div key={r.xp} style={{
                  ...
                  <div style={{ fontSize: 24 }}>{unlocked ? r.emoji : "🔒"}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 15, color: unlocked ? "#f5f0e8" : "#666" }}>{r.name}</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "18px 22px" }}>
          <div style={{ fontSize: 11, letterSpacing: 3, color: "#888", textTransform: "uppercase", marginBottom: 12 }}>XP Guide</div>
